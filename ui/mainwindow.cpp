...
MainWindow::MainWindow(QWidget *parent, const QStringList &fileList)
    : QMainWindow(parent), ui(new Ui::MainWindow), m_presets(new Presets(this)),
      m_list(new ConvertList(m_presets, this)), m_argv_input_files(fileList),
      m_elapsedTimeLabel(new QLabel(this)), m_timer(new QTimer(this)),
      m_poweroff_button(nullptr), m_poweroff_actiongroup(nullptr),
      m_update_checker(new UpdateChecker(this)) {
  ui->setupUi(this);

  connect(m_list, SIGNAL(task_finished(int)), this, SLOT(task_finished(int)));
  connect(m_list, SIGNAL(all_tasks_finished()), this,
          SLOT(all_tasks_finished()));
  connect(m_list, SIGNAL(customContextMenuRequested(QPoint)), this,
          SLOT(slotListContextMenu(QPoint)));
  connect(m_list, SIGNAL(itemSelectionChanged()), this,
          SLOT(refresh_action_states()));
  connect(m_timer, SIGNAL(timeout()), this, SLOT(timerEvent()));
  connect(m_list, SIGNAL(started()), this, SLOT(conversion_started()));
  connect(m_list, SIGNAL(stopped()), this, SLOT(conversion_stopped()));
  connect(m_update_checker, SIGNAL(receivedResult(int)), this,
          SLOT(received_update_result(int)));
...
void MainWindow::task_finished(int exitcode) {
  if (exitcode == 0) { // succeed
    ...
    Notification::send(this, "MystiQ", tr("Conversion finished successfully."),
                       NotifyLevel::INFO);
#endif
  } else { // failed
    QMessageBox::critical(this, this->windowTitle(), tr("Conversion failed."),
                          QMessageBox::Ok);
  }
}

void MainWindow::all_tasks_finished() {
  Notification::send(this, "MystiQ", tr("All tasks have finished."),
                     NotifyLevel::INFO);
  ...
  if (PowerManagement::implemented() && m_poweroff_button->isChecked()) {
    ...
    if (PoweroffDialog(this).exec(get_poweroff_behavior()) ==
        QDialog::Accepted) {
...
void MainWindow::slotAddFiles() { add_files(); }
...
void MainWindow::slotExit() { this->close(); }
...
void MainWindow::slotStopConversion() { m_list->stop(); }
...
void MainWindow::slotAboutQt() { QMessageBox::aboutQt(this); }

void MainWindow::slotAboutFFmpeg() { AboutFFmpegDialog(this).exec(); }

void MainWindow::slotAbout() { AboutDialog(this).exec(); }

void MainWindow::slotHelpMystiQDialog() { HelpMystiQDialog(this).exec(); }

void MainWindow::slotDonate() {
  QMessageBox d(this);
  d.setWindowTitle(tr("Support MystiQ"));
  QPushButton *liberapay_button =
      d.addButton("Liberapay", QMessageBox::YesRole);
  QPushButton *opencollective_button =
      d.addButton("Open Collective", QMessageBox::YesRole);
  ...
  d.setText("<h1>" + tr("MystiQ needs you") + "</h1><p>" +
            tr("MystiQ is free software. However the development requires a "
               "lot of time and a lot of work. In order to keep developing "
               "MystiQ with new features we need your help. Please consider to "
               "support the MystiQ project by sending a donation. <b>Even the "
               "smallest amount will help a lot.</b>"));
  d.exec();
...
void MainWindow::slotTranslate() {
  QString translateWeb =
      "https://www.transifex.com/swl-x-project/mystiq-video-converter/";
  bool b = QDesktopServices::openUrl(QUrl(translateWeb, QUrl::TolerantMode));
  Q_UNUSED(b)
}

void MainWindow::slotOpenSettingFolder() {
  QDesktopServices::openUrl(QUrl(Paths::appPath()));
}
...
void MainWindow::slotReport() {
  ...
  d.setText(
      "<h1>" + tr("We need your feedback") + "</h1><p>" +
      tr("MystiQ Video Converter development team needs users who use the "
         "application to help detect errors. If you find an error and report "
         "it to our team, it will help speed up the correction process. "
         "Therefore, you can be part of our development team even if you are "
         "not a developer. To report an error, you can send us an email with "
         "the detailed description of it or you can enter Github and describe "
         "the issue (in this case you must have a GitHub Account)."));
  d.exec();
  if (d.clickedButton() == email_button) {
    ...
    stringList << "?";
    stringList << "subject=";
    stringList << QString(tr("Reporting bugs from MystiQ ") +
                          tr("%1").arg(VERSION_STRING));
    stringList << "&";
    stringList << "body=";
...
void MainWindow::slotCut() { m_list->cutSelectedTask(); }
...
void MainWindow::timerEvent() { refresh_status(); }
...
void MainWindow::update_poweroff_button(int id) {
  ...
  default:
    Q_ASSERT_X(
        false, __FUNCTION__,
        "Incorrect id! Be sure to handle every power action in switch().");
  }
...
void MainWindow::received_update_result(int status) {
  if (status == UpdateChecker::UpdateFound) {
    ...
    const bool timeout =
        !timer.isValid() || timer.elapsedSeconds() > seconds_per_day;
    const bool is_different_version = new_update_version != prev_update_version;
...
bool MainWindow::ask_for_update_permission() {
  ...
  int reply = QMessageBox::information(this, windowTitle(), msg,
...
void MainWindow::add_files() {
  ...
  if (wizard.exec_openfile() == QDialog::Accepted) {
    ...
    const QList<ConversionParameters> &paramList =
        wizard.getConversionParameters();
...
void MainWindow::setup_menus() {
  ...
  // File
  connect(ui->actionAddFiles, SIGNAL(triggered()), this, SLOT(slotAddFiles()));
  connect(ui->actionOptions, SIGNAL(triggered()), this, SLOT(slotOptions()));
  connect(ui->actionExit, SIGNAL(triggered()), this, SLOT(slotExit()));

  // Edit
  connect(ui->menuEdit, SIGNAL(aboutToShow()), this,
          SLOT(refresh_action_states()));
  connect(ui->actionRemoveSelectedItems, SIGNAL(triggered()), m_list,
          SLOT(removeSelectedItems()));
  connect(ui->actionRemoveCompletedItems, SIGNAL(triggered()), m_list,
          SLOT(removeCompletedItems()));
  connect(ui->actionClearList, SIGNAL(triggered()), m_list, SLOT(clear()));
  connect(ui->actionSetParameters, SIGNAL(triggered()), this,
          SLOT(slotSetConversionParameters()));
  connect(ui->actionOpenOutputFolder, SIGNAL(triggered()), this,
          SLOT(slotOpenOutputFolder()));
  connect(ui->actionOpenSettingFolder, SIGNAL(triggered()), this,
          SLOT(slotOpenSettingFolder()));
  connect(ui->actionChangeOutputFilename, SIGNAL(triggered()), m_list,
          SLOT(changeSelectedOutputFile()));
  connect(ui->actionChangeOutputDirectory, SIGNAL(triggered()), m_list,
          SLOT(changeSelectedOutputDirectory()));
  connect(ui->actionShowErrorMessage, SIGNAL(triggered()), m_list,
  ...
  // Convert
  connect(ui->menuConvert, SIGNAL(aboutToShow()), this,
          SLOT(refresh_action_states()));
  connect(ui->actionStartConversion, SIGNAL(triggered()), this,
          SLOT(slotStartConversion()));
  connect(ui->actionStopConversion, SIGNAL(triggered()), this,
          SLOT(slotStopConversion()));
  connect(ui->actionRetry, SIGNAL(triggered()), m_list,
          SLOT(retrySelectedItems()));
  connect(ui->actionRetry, SIGNAL(triggered()), this,
          SLOT(refresh_action_states()));
  connect(ui->actionRetryAll, SIGNAL(triggered()), m_list, SLOT(retryAll()));
  connect(ui->actionRetryAll, SIGNAL(triggered()), this,
          SLOT(refresh_action_states()));

  // About
  connect(ui->actionHelpMystiQDialog, SIGNAL(triggered()), this,
          SLOT(slotHelpMystiQDialog()));
  connect(ui->actionAboutQt, SIGNAL(triggered()), this, SLOT(slotAboutQt()));
  connect(ui->actionAboutFFmpeg, SIGNAL(triggered()), this,
          SLOT(slotAboutFFmpeg()));
  connect(ui->actionAbout, SIGNAL(triggered()), this, SLOT(slotAbout()));
  connect(ui->actionReport, SIGNAL(triggered()), this, SLOT(slotReport()));
  connect(ui->actionDonate, SIGNAL(triggered()), this, SLOT(slotDonate()));
  connect(ui->actionTranslate, SIGNAL(triggered()), this,
          SLOT(slotTranslate()));
  connect(ui->actionCheckUpdate, SIGNAL(triggered()), this,
          SLOT(slotShowUpdateDialog()));
}
...
#define ADD_ACTION(name)                                                       \
  toolbar_table[QString(#name).toUpper()] = ui->action##name
  ADD_ACTION(AddFiles);
  ADD_ACTION(Options);
  ADD_ACTION(Exit);
...
void MainWindow::setup_poweroff_button() {
  ...
  for (int i = 0; i < PowerManagement::ACTION_COUNT; i++) {
    ...
    default:
      Q_ASSERT_X(
          false, __FUNCTION__,
          "Incorrect id! Be sure to implement every power action in switch().");
    }

    QIcon icon(icon_id);

    actionList.append(new QAction(QIcon(icon), text, this));
  }
   ...
   */
  connect(button, SIGNAL(toggled(bool)), ui->actionPoweroff,
          SLOT(setChecked(bool)));
  connect(ui->actionPoweroff, SIGNAL(toggled(bool)), button,
          SLOT(setChecked(bool)));
  ...
  for (int i = 0; i < actionList.size(); i++) {
    ...
    connect(action, &QAction::triggered,
            [this, i] { update_poweroff_button(i); });
  }
...
bool MainWindow::load_presets() {
  ...
  if (!Constants::getBool("Portable")) { // non-portable app
    ...
    QString local_preset_file_old =
        QDir(QDir::homePath()).absoluteFilePath(".mystiq/presets.xml");
    if (QFile(local_preset_file_old).exists()) {
      QFile::remove(local_preset_file_old + ".old");
      if (QFile::rename(local_preset_file_old,
                        local_preset_file_old + ".old")) {
        qDebug() << local_preset_file_old +
                        " is no longer used, "
                        "rename to " +
                        local_preset_file_old + ".old";
      }
...
void MainWindow::refresh_action_states() {
   ...
   */
  bool hide_ShowErrorMessage =
      (selected_file_count != 1 || !m_list->selectedTaskFailed());
  ...
  ui->actionCut->setEnabled(selected_file_count ==
                            1); // cut only 1 file at a time
}
...
void MainWindow::load_settings() {
  ...
  int poweroff_behavior =
      settings.value("options/poweroff_behavior", PowerManagement::SHUTDOWN)
          .toInt();
...
void MainWindow::refresh_statusbar() {
  if (m_list->isBusy()) {
    ...
    QString timeinfo =
        tr("Elapsed Time: %1 h %2 m %3 s").arg(hours).arg(minutes).arg(seconds);
